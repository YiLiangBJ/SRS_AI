...
class SRSTrainerUnifiedMMSE:
    """
    Trainer for SRS Channel Estimator using h_with_residual/phasor as MMSE input
    """
    def __init__(
        self,
        config: SRSConfig,
        device: str = "cuda" if torch.cuda.is_available() else "cpu",
        save_dir: str = "./checkpoints_unified_mmse",
        ...
        # Make sure model has trainable parameters
    ...
    def train_epoch(self, num_batches: int, batch_size: int) -> Tuple[float, float]:
        ...
        for batch_idx in tqdm(range(num_batches), desc="训练中"):
            ...
            for i in range(batch_size):                
                ...
                # Process through SRS estimator first to get h_with_residual/phasor
                # We need to run this without the trainable MMSE initially
                # to get the h_with_residual/phasor value
                with torch.no_grad():
                    # Temporarily disable MMSE matrices
                    old_C = self.srs_estimator.C_matrix
                    old_R = self.srs_estimator.R_matrix
                    self.srs_estimator.C_matrix = None
                    self.srs_estimator.R_matrix = None
                    
                    # Run through estimator to generate h_with_residual/phasor
                    self.srs_estimator(
                    ...
                    # Restore MMSE matrices
                    self.srs_estimator.C_matrix = old_C
                    self.srs_estimator.R_matrix = old_R
                
                # Use trainable MMSE module with h_with_residual/phasor as input
                if self.mmse_module and hasattr(self.srs_estimator, 'current_h_with_residual_phasor'):
                    channel_input = self.srs_estimator.current_h_with_residual_phasor
                    if channel_input is not None:
                        C, R, _ = self.mmse_module(channel_input)
                        self.srs_estimator.set_mmse_matrices(C=C, R=R)
                
                # Process through SRS estimator with updated MMSE matrices
        ...
        with torch.no_grad():
            for batch_idx in tqdm(range(num_batches), desc="验证中"):
                ...
                for i in range(batch_size):
                    ...
                    # Get h_with_residual/phasor first
                    # Temporarily disable MMSE matrices
                    old_C = self.srs_estimator.C_matrix
                    old_R = self.srs_estimator.R_matrix
                    self.srs_estimator.C_matrix = None
...
def main():
    ...
    parser.add_argument('--save_dir', type=str, default='./checkpoints_unified_mmse', help='Directory for saving checkpoints')
