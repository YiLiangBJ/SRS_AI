...
---2. [test_separator.py 使用指南](#test_separatorpy-使用指南)python ./Model_AIIC/channel_separator.py
...
在 5G NR SRS 系统中，多个端口的信道响应在时域混叠：5. [CPU 性能优化](#cpu-性能优化)python ./Model_AIIC/test_separator.py --model mlp --epochs 10



```6. [模型部署](#模型部署)python ./Model_AIIC/test_separator.py --model residual --epochs 20

y = sum(circshift(h_p, pos_p)) + noise

7. [故障排查](#故障排查)python ./Model_AIIC/test_separator.py --model hinted --epochs 10
