...
# 3. 绘制曲线See `INTEGRATION_GUIDE.md` for detailed integration steps with existing codebase.
...
python Model_AIIC/evaluate_models.py \查看所有实验的超参数和最终性能：

  --exp_dir <训练目录> \

  --tdl <TDL配置> \| num_stages | share_weights | batch_size | snr_db | test_nmse_db | final_train_loss |

  --snr_range <SNR范围> \|------------|---------------|------------|--------|--------------|------------------|

  --num_batches <批次数> \| 2          | False         | 2048       | 20.0   | -15.2        | 0.045            |

  --batch_size <批大小> \| 3          | False         | 2048       | 20.0   | -16.5        | 0.038            |

  --output <输出目录>| 3          | True          | 2048       | 20.0   | -15.9        | 0.041            |
...
| `--models` | 所有 | 指定模型 (逗号分隔) |#### 3. Graphs（计算图）

| `--tdl` | "A-30,B-100,C-300" | TDL 配置 |

| `--snr_range` | "30:-3:0" | SNR 范围 |可视化模型的网络结构和计算流程。

| `--num_batches` | 10 | 批次数 |

| `--batch_size` | 100 | 批大小 |### TensorBoard 使用技巧
...
python Model_AIIC/evaluate_models.py \---
  ...
  --tdl "A-30,B-100,C-300" \## 🔍 超参数搜索

  --snr_range "30:-3:0" \

  --num_batches 10 \### 单个超参数

  --batch_size 200 \

  --output ./full_eval```bash
...
python Model_AIIC/plot_results.py \```
  ...
  --filter_tdl <TDL过滤>| CPU 核心数 | 推荐 batch_size | 说明 |

```|-----------|----------------|------|

| 4-8       | 512-1024       | 小规模 |

### 6.2 布局类型| 8-16      | 1024-2048      | 中等规模 |

| 16-56     | 2048-4096      | 大规模 |

#### single - 单图| 56+       | 4096-8192      | 服务器级 |
...
python Model_AIIC/plot_results.py \  --batch_size 4096 \

  --input ./full_eval \  --cpu_ratio 1.0
...
每个 TDL 一个子图# 方法 1: 环境变量（最高优先级）

export OMP_NUM_THREADS=8

```bashpython Model_AIIC/test_separator.py  # 使用 8 线程
