...
def export_runs_to_onnx(
    ...
    if output_root is not None and len(target_dirs) > 1:
        raise ValueError('Shared output_root is only supported for a single run. For multiple runs, omit --output so each run writes to its own onnx_exports directory.')
    return [
        export_run_to_onnx(
            run_dir=target_dir,
