...
def _save_latency_samples(results: List[Dict[str, Any]], output_dir: Path) -> Path:
    samples_payload = {
        f"{item['run_name']}__{item['device']}__{item['requested_precision_profile']}__bs{item['batch_size']}__t{item['num_threads']}": np.asarray(item.get('latency_samples_ms', []), dtype=np.float64)
        for item in results
...
def _plot_run_latency(results: List[Dict[str, Any]], output_dir: Path) -> List[Path]:
    ...
    for item in ok_results:
        grouped.setdefault((item['requested_precision_profile'], item['num_threads']), []).append(item)

    fig, axis = plt.subplots(figsize=(12, 7))
    for (precision, num_threads), items in sorted(grouped.items()):
        ...
        axis.plot(
            ...
            marker='o',
            linewidth=2,
            label=f'{precision} / t={num_threads}',
        )
    ...
    thread_candidates = {(item['requested_precision_profile'], item['batch_size']) for item in ok_results if item['num_threads'] != 1}
    if thread_candidates:
        fig, axis = plt.subplots(figsize=(12, 7))
        for precision, batch_size in sorted(thread_candidates):
            items = sorted(
                [entry for entry in ok_results if entry['requested_precision_profile'] == precision and entry['batch_size'] == batch_size],
            ...
            axis.plot(
                ...
                marker='o',
                linewidth=2,
                label=f'{precision} / bs={batch_size}',
            )
...
def render_latency_report(results_payload: Dict[str, Any]) -> str:
    ...
    if cpu_thread_summaries:
        ...
        for run_name, summary in cpu_thread_summaries.items():
    ...
    for item in results_payload['results']:
        lines.append(
            f"| `{item['run_name']}` | `{item['requested_precision_profile']}` | {item['batch_size']} | {item['num_threads']} | {item['status']} | "
            f"{item.get('p50_latency_ms', '-')} | {item.get('p95_latency_ms', '-')} | {item.get('p99_latency_ms', '-')} | {item.get('throughput_samples_per_sec', '-')} | {item.get('skip_reason', '-') or '-'} |"
        )
    return '\n'.join(lines) + '\n'
