...
| `real_branch_hidden_1` | `[-1, 32]` | `per port, per stage (2 stages total, stage weights shared)` | `800` | `4,800` | The first branch affine layer maps width 24 to hidden width 32. |
| `imag_branch_hidden_1` | `[-1, 32]` | `per port, per stage (2 stages total, stage weights shared)` | `800` | `4,800` | The first branch affine layer maps width 24 to hidden width 32. |
| `real_branch_output` | `[-1, 12]` | `per port, per stage (2 stages total, stage weights shared)` | `396` | `2,376` | The real branch final affine layer reduces hidden width 32 to one real channel width 12. |
| `imag_branch_output` | `[-1, 12]` | `per port, per stage (2 stages total, stage weights shared)` | `396` | `2,376` | The imaginary branch final affine layer reduces hidden width 32 to one imaginary channel width 12. |
