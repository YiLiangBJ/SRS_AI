...
%   outputData = predict_refactor_matlab_bundle(".../<run_name>/matlab_model_bundle.mat")
...
switch string(modelSpec.model_type)
    ...
    case "separator3"
        [outputData, debug] = local_forward_separator3(weights, modelSpec, normalizedInput, collectDetailedDebug);
    case "full_mlp"
...
function [outputData, debug] = local_forward_separator3(weights, modelSpec, inputData, collectDetailedDebug)
numPorts = double(modelSpec.num_ports);
seqLen = double(modelSpec.seq_len);
numStages = double(modelSpec.num_stages);
...
for stageIdx = 1:numStages
    ...
    while isfield(weights, sprintf('stage%02d_joint_l%02d_weight', stageIdx, layerIdx))
        prefix = sprintf('stage%02d_joint_l%02d', stageIdx, layerIdx);
        weight = single(weights.([prefix '_weight']));
        bias = single(weights.([prefix '_bias']));
        layerInput = x;
        affine = layerInput * weight.' + bias;
        x = affine;
        if isfield(weights, sprintf('stage%02d_joint_l%02d_weight', stageIdx, layerIdx + 1))
            x = max(x, 0);
        end

        if collectDetailedDebug
            layerTrace{end + 1} = struct( ...
                'layer_index', layerIdx, ...
                'prefix', prefix, ...
                'input', layerInput, ...
                'weight', weight, ...
    ...
    switch residualMode
        case "learned_dense"
            maskName = sprintf('stage%02d_residual_mask', stageIdx);
            if ~isfield(weights, maskName)
                error("predict_refactor_matlab_bundle:MissingSeparator3Mask", ...
                    "Missing learned residual mask for stage %d.", stageIdx);
            end
            residualMask = single(weights.(maskName));
            features = features + reshape(residual, [size(inputData, 1), 1, seqLen * 2]) .* reshape(residualMask, [1, numPorts, seqLen * 2]);
        case "masked"
...
outputData = features;
debug = struct();
debug.model_type = "separator3";
debug.stage_outputs = stageOutputs;
if collectDetailedDebug
    debug.stage_layer_traces = stageTraces;
end
end
...
function value = local_manifest_field(structValue, fieldName, defaultValue)
if isstruct(structValue) && isfield(structValue, fieldName)
