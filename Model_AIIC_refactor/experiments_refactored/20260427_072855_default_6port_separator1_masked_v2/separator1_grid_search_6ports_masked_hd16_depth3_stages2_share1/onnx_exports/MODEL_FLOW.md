...
| `real_branch_hidden_1` | `[-1, 16]` | `per port, per stage (2 stages total, stage weights shared)` | `400` | `2,400` | The first branch affine layer maps width 24 to hidden width 16. |
| `imag_branch_hidden_1` | `[-1, 16]` | `per port, per stage (2 stages total, stage weights shared)` | `400` | `2,400` | The first branch affine layer maps width 24 to hidden width 16. |
| `real_branch_output` | `[-1, 12]` | `per port, per stage (2 stages total, stage weights shared)` | `204` | `1,224` | The real branch final affine layer reduces hidden width 16 to one real channel width 12. |
| `imag_branch_output` | `[-1, 12]` | `per port, per stage (2 stages total, stage weights shared)` | `204` | `1,224` | The imaginary branch final affine layer reduces hidden width 16 to one imaginary channel width 12. |
