...
- Output shape: `[-1, 6, 24]`
- Total trainable parameters: `510,240`
...
| `port_stage_input` | `[-1, 24]` | `per port, per stage (2 stages total)` | `0` | `0` | Each port-stage block consumes one real-stacked sequence of width 24. |
| `real_branch_hidden_1` | `[-1, 128]` | `per port, per stage (2 stages total)` | `3,200` | `38,400` | The first branch affine layer maps width 24 to hidden width 128. |
| `imag_branch_hidden_1` | `[-1, 128]` | `per port, per stage (2 stages total)` | `3,200` | `38,400` | The first branch affine layer maps width 24 to hidden width 128. |
| `real_branch_output` | `[-1, 12]` | `per port, per stage (2 stages total)` | `1,548` | `18,576` | The real branch final affine layer reduces hidden width 128 to one real channel width 12. |
| `imag_branch_output` | `[-1, 12]` | `per port, per stage (2 stages total)` | `1,548` | `18,576` | The imaginary branch final affine layer reduces hidden width 128 to one imaginary channel width 12. |
| `port_output` | `[-1, 24]` | `per port, per stage (2 stages total)` | `0` | `0` | Concatenating one real width-12 output and one imag width-12 output reconstructs one width-24 port tensor. |
