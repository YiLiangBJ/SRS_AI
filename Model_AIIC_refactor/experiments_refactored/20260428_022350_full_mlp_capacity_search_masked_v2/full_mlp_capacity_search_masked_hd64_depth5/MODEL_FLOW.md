...
- Output shape: `[-1, 6, 24]`
- Total trainable parameters: `19,280`
...
| `joint_hidden_1` | `[-1, 64]` | `-` | `1,600` | `1,600` | The first affine layer maps the input width 24 into the configured hidden width 64. |
| `joint_hidden_1_relu` | `[-1, 64]` | `-` | `0` | `0` | ReLU is elementwise, so it keeps the hidden shape unchanged. |
| `joint_hidden_2` | `[-1, 64]` | `-` | `4,160` | `4,160` | This affine layer keeps the same hidden width 64 while adding capacity. |
| `joint_hidden_2_relu` | `[-1, 64]` | `-` | `0` | `0` | ReLU is elementwise, so the hidden width stays the same. |
| `joint_hidden_3` | `[-1, 64]` | `-` | `4,160` | `4,160` | This affine layer keeps the same hidden width 64 while adding capacity. |
| `joint_hidden_3_relu` | `[-1, 64]` | `-` | `0` | `0` | ReLU is elementwise, so the hidden width stays the same. |
| `joint_linear_output` | `[-1, 144]` | `-` | `9,360` | `9,360` | The output affine layer expands hidden width 64 to flat joint output width 144. |
